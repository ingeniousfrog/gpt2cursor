...
pub fn load_tray_icon() -> Result<Image<'static>, String> {
    ...
    let cropped = crop_to_content(decoded.to_rgba8(), 0.12);
    let fitted = resize_to_fit(cropped, TRAY_PX);
    let masked = mask_rounded_corners(fitted, CORNER_RADIUS);
    Ok(Image::new_owned(masked.into_raw(), TRAY_PX, TRAY_PX))
}

fn crop_to_content(img: RgbaImage, padding_ratio: f32) -> RgbaImage {
    ...
    let pad_x = (content_w as f32 * padding_ratio).round() as u32;
    let pad_y = (content_h as f32 * padding_ratio).round() as u32;
